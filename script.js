...
const products = [
  {
    ...
    price:127000,
    images:["aspiradora-1.jpg","aspiradora-2.jpg","aspiradora-3.jpg","aspiradora-4.jpg"],
  ...
  },
  {
    ...
    price:89999,
    images:["linterna-1.jpg","linterna-2.jpg"],
  ...
  },
  {
    ...
    price:269000,
    images:["instax-1.jpg","instax-2.jpg","instax-3.jpg","instax-4.jpg"],
...
function setMain(src){
  ...
  document.querySelectorAll("#thumbs img").forEach(x =>
    x.classList.toggle("active",x.dataset.img===src)
  );
}
...
function renderCart(){
  ...
  if(!cart.length){
    el.innerHTML='<p style="color:#888;padding:25px 0">Tu carrito está vacío.</p>';
...
function openCart(){
  document.querySelector("#cartPanel").classList.remove("hidden");
...
document.addEventListener("click",e=>{
  ...
  if(thumb)setMain(thumb.dataset.img);
  ...
  if(dec){
    ...
    if(i && i.qty<=0){
      cart=cart.filter(x=>x.id!==dec.dataset.dec);
    }
